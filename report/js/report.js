...
function StepPannel(data, root){
  ...
  this.getStepRightArgs = function(step){
        ...
        for(var i=0; i < step.code.args.length; i++){
          ...
          if(arg.image){
            argHtml += ('<img class="crop_image desc" data-width="%s" data-height="%s" src="%s" title="%s">' +
                    '<dir class="desc">resolution: %s</dir>')
            ...
            argHtml += '<dir class="desc">%s: %s</dir>'.format(arg.key,val)
          }
        }
      }
    } catch(e) {
    ...
    if(step.screen && step.screen.confidence){
      argHtml += '<dir class="desc"><span class="point glyphicon glyphicon-play"></span><span lang="en">Confidence: </span>%s</dir>'.format(step.screen.confidence)
    }

    argHtml =  argHtml || '<dir class="desc">None</dir>'
  ...
  this.getStepRightTrace = function(step){
    if(step.traceback){
      return '<dir class="desc"><pre class="trace"><code class="python">%s</code></pre></dir>'.format(step.traceback)
    } else{
      return ""
    }
  }
